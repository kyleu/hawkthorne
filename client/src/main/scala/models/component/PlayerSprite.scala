...
class PlayerSprite(
  ...
  as.sprite.name = s"${player.idx}.${player.templateKey}.${player.costume.key}"
  ...
  def displayBounds(rect: util.Rectangle) = {
    ...
    val i = StaticImage(game = game, group = mapGroup, name = s"player.${player.idx}.bounds.debug", tex = t)
