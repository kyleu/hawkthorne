...
import models.ddl.SchemaMigrationResult
import models.result.orderBy.OrderBy
import play.api.http.MimeTypes
import scala.concurrent.Future
import services.audit.AuditRecordService
import services.ddl.SchemaMigrationService
import util.JsonSerializers._
import util.ReftreeUtils._

@javax.inject.Singleton
class SchemaMigrationController @javax.inject.Inject() (
    override val app: Application, svc: SchemaMigrationService, auditRecordSvc: AuditRecordService
  ...
  def create = withSession("create", admin = true) { implicit request => implicit td =>
    svc.create(request, modelForm(request.body)).map {
      case Some(model) => Redirect(controllers.admin.ddl.routes.SchemaMigrationController.view(model.installedRank))
      case None => Redirect(controllers.admin.ddl.routes.SchemaMigrationController.list())
    }
  }
  ...
  def list(q: Option[String], orderBy: Option[String], orderAsc: Boolean, limit: Option[Int], offset: Option[Int], t: Option[String] = None) = {
    withSession("list", admin = true) { implicit request => implicit td =>
      ...
      searchWithCount(q, orderBys, limit, offset).map(r => renderChoice(t) {
        case MimeTypes.HTML => Ok(views.html.admin.ddl.schemaMigrationList(
          request.identity, Some(r._1), r._2, q, orderBy, orderAsc, limit.getOrElse(100), offset.getOrElse(0)
        ))
        case MimeTypes.JSON => Ok(SchemaMigrationResult.fromRecords(q, Nil, orderBys, limit, offset, startMs, r._1, r._2).asJson)
        case ServiceController.MimeTypes.csv => csvResponse("SchemaMigration", svc.csvFor(r._1, r._2))
  ...
  def view(installedRank: Long, t: Option[String] = None) = withSession("view", admin = true) { implicit request => implicit td =>
    ...
    val notesF = app.coreServices.notes.getFor(request, "schemaMigration", installedRank)

    notesF.flatMap(notes => modelF.map {
      case Some(model) => renderChoice(t) {
        case MimeTypes.HTML => Ok(views.html.admin.ddl.schemaMigrationView(request.identity, model, notes, app.config.debug))
        case MimeTypes.JSON => Ok(model.asJson)
        case ServiceController.MimeTypes.png => Ok(renderToPng(v = model)).as(ServiceController.MimeTypes.png)
        case ServiceController.MimeTypes.svg => Ok(renderToSvg(v = model)).as(ServiceController.MimeTypes.svg)
      }
      case None => NotFound(s"No SchemaMigration found with installedRank [$installedRank].")
    })
  }

  def editForm(installedRank: Long) = withSession("edit.form", admin = true) { implicit request => implicit td =>
    val cancel = controllers.admin.ddl.routes.SchemaMigrationController.view(installedRank)
    val call = controllers.admin.ddl.routes.SchemaMigrationController.edit(installedRank)
    svc.getByPrimaryKey(request, installedRank).map {
      case Some(model) => Ok(
        views.html.admin.ddl.schemaMigrationForm(request.identity, model, s"Schema Migration [$installedRank]", cancel, call, debug = app.config.debug)
      )
      case None => NotFound(s"No SchemaMigration found with installedRank [$installedRank].")
    }
  }
  ...
  def edit(installedRank: Long) = withSession("edit", admin = true) { implicit request => implicit td =>
    svc.update(request, installedRank = installedRank, fields = modelForm(request.body)).map(res => render {
      case Accepts.Html() => Redirect(controllers.admin.ddl.routes.SchemaMigrationController.view(res._1.installedRank)).flashing("success" -> res._2)
  ...
  def remove(installedRank: Long) = withSession("remove", admin = true) { implicit request => implicit td =>
    svc.remove(request, installedRank = installedRank).map(_ => render {
      case Accepts.Html() => Redirect(controllers.admin.ddl.routes.SchemaMigrationController.list())
