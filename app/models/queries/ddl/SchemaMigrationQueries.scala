...
object SchemaMigrationQueries extends BaseQueries[SchemaMigration]("schemaMigration", "flyway_schema_history") {
  ...
  def insert(model: SchemaMigration) = new Insert(model)
  def insertBatch(models: Seq[SchemaMigration]) = new InsertBatch(models)
  ...
  override def fromRow(row: Row) = SchemaMigration(
